...
if (!modes || Object.keys(modes).length < 1) return console.error('No modes found')

fs.readFile(require.resolve('../run.sh'), 'utf8', function (err, data) {
	let re = new RegExp('read -p "Mode (.+): " mode', 'gi')
	let formatted = data.replace(re, 'read -p "Mode (' + Object.keys(modes).join(', ') + '): " mode')
...
fs.readFile(require.resolve('../run.bat'), 'utf8', function (err, data) {
	...
	let formatted = data.replace(re, 'set /p "mode=Mode (' + Object.keys(modes).join(', ') + '): "')
